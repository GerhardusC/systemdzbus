...
impl<'a> SystemCtl<'a> {
    ...
    pub async fn reload_unit(&self, name: &str, mode: UnitMode) -> Result<OwnedObjectPath, SystemdError> {
        Ok(self.get_manager_proxy().reload_unit(name, &mode.to_string()).await?)
    }
